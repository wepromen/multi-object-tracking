...
#         image = Image.fromarray(args["frame"])
        
#         ## yolo detection
#         boxs = args["yolo"].detect_image(image) # [x,y,w,h]
#         # print("\n @@ box_num",len(boxs))
#         features = args["encoder"](args["frame"],boxs)
...
#         indices = preprocessing.non_max_suppression(boxes, args["nms_max_overlap1"], scores)
#         detections = [detections[i] for i in indices]

#         ### Call the tracker
#         args["tracker"].predict()
#         args["tracker"].update(detections)
...
#             oldframe = args["frame"]
#         else:
#             startXs,startYs = getFeatures(cv2.cvtColor(args["frame"],cv2.COLOR_RGB2GRAY),bboxs,use_shi=False)
#             newXs, newYs = estimateAllTranslation(startXs, startYs, oldframe, args["frame"])
...
#         boxes_tracking = np.array([track.to_tlwh() for track in args["tracker"].tracks])
#         ### Deep sort tracker visualization
#         for track in args["tracker"].tracks:
#             if not track.is_confirmed() or track.time_since_update > 1:
#                 continue 
#             bbox = track.to_tlbr()
#             cv2.rectangle(args["frame"], (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])),(255,255,255), 2)
#             cv2.putText(args["frame"], str(track.track_id),(int(bbox[0]), int(bbox[1])),0, 5e-3 * 200, (0,255,0),2)
...
#                     startXs,startYs = getFeatures(cv2.cvtColor(args["frame"],cv2.COLOR_RGB2GRAY),newbboxs)
...
#                     cv2.rectangle(args["frame"], (xmin,ymin), (xmin+boxw,ymin+boxh), (255,255,255), 2) # BGR color
#                     cv2.putText(args["frame"], str(j),(xmin,ymin),0, 5e-3 * 200, (0,255,0),2)
#                     # red color features
...
#         for det in detections:
#             bbox = det.to_tlbr()
#             cv2.rectangle(args["frame"],(int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])),(255,0,0), 2) # BGR color
...
#         cv2.putText(args["frame"], textFPS, (10, 450), font, 0.5, (255, 0, 255), 2)    
...
def main(yolo):
    ...
    videoShowerProcess = Process(target=videoShower.start, args=(showerVideo, showerFrameQueue, showerBBQueue))
    ...
    while True:
        ...
        # args = {"frame": frame, "yolo": yolo, "encoder": encoder, "nms_max_overlap1": nms_max_overlap1, \
        #         "tracker": tracker,"OPTICAL": OPTICAL,"firstflag": firstflag,"fps": fps}
        ...
        for track in tracker.tracks:
            if not track.is_confirmed() or track.time_since_update > 1:
                continue 
            bbox = track.to_tlbr()
            # showerBBQueue.put((int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])))
